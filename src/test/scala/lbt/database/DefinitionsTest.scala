...
class DefinitionsTest extends fixture.FunSuite with ScalaFutures {
  ...
  test("Route definitions for existing bus number 3 can be loaded from DB") { f =>
    ...
    busDefinitions(f.testBusRoute1).count(stop => stop.name.value.contains("Brixton")) should be > 0
    busDefinitions(f.testBusRoute2).count(stop => stop.name.value.contains("Brixton")) should be > 0

  }

  test("Bus Route not already in DB is reloaded from web on request") { f =>
    f.testDefinitionsCollection.getBusRouteDefinitions()
    val newBusRoute = BusRoute(RouteID("521"), Direction("inbound"))
    ...
    f.testDefinitionsCollection.getBusRouteDefinitions()(newBusRoute).count(stop => stop.name.value.contains("London Bridge")) should be > 0
  }

  test("Sequence is kept in order when loaded from web and retrieved from db") { f =>
    val busDefinitions = f.testDefinitionsCollection.getBusRouteDefinitions()
    busDefinitions.get(f.testBusRoute1) shouldBe defined
    busDefinitions(f.testBusRoute1).head.name.value should include("Conduit Street")
    busDefinitions(f.testBusRoute1).last.name.value should include("Crystal Palace")
  }
}
