...
class HistoricalRecorderTest extends fixture.FunSuite with ScalaFutures with Eventually {
  ...
  test("Line should be accepted if route is in definitions or disregarded if not in definitions") { f =>
    ...
    val validSourceline = "[1,\"" + randomStop.id.value + "\",\"" + f.testBusRoute1.id.value + "\",1,\"Any Place\",\"SampleReg\"," + f.generateArrivalTime + "]"
    val invalidSourceLine = "[1,\"" + randomStop.id.value + "\",\"99XXXX\",2,\"Bromley North\",\"YX62DYN\"," + f.generateArrivalTime + "]"
  ...
  test("Line should be accepted if bus stop is in definitions and rejected if not") { f =>
    ...
    val validSourceline = "[1,\"" + randomStop.id.value + "\",\"" + f.testBusRoute1.id.value + "\",1,\"Any Place\",\"SampleReg\"," + f.generateArrivalTime  + "]"
    val invalidSourceLine = "[1,\"" + "XXSTOP" + "\",\"" + f.testBusRoute1.id.value + "\",1,\"Any Place\",\"SampleReg\"," + f.generateArrivalTime  + "]"
    val testLines = List(validSourceline, invalidSourceLine)
  ...
  test("Lines received with arrival time in past should be ignored"){f=>
    ...
    val testLines: List[String] = routeDefFromDb.map(busStop =>
      "[1,\"" + busStop.id.value + "\",\"" + f.testBusRoute1.id.value + "\",1,\"Any Place\",\"V123456\"," + f.generateArrivalTime + "]")
  ...
  test("An actor should be created for each new vehicle received with a new route") { f=>
    ...
    val validSourceline1 = "[1,\"" + randomStopRoute1.id.value + "\",\"" + f.testBusRoute1.id.value + "\",1,\"Any Place\",\"V123456\"," + f.generateArrivalTime + "]"
    val validSourceline2 = "[1,\"" + randomStopRoute1.id.value + "\",\"" + f.testBusRoute1.id.value + "\",1,\"Any Place\",\"V123456\"," + f.generateArrivalTime + "]"
    val validSourceline3 = "[1,\"" + randomStopRoute1.id.value + "\",\"" + f.testBusRoute1.id.value + "\",1,\"Any Place\",\"V23456\"," + f.generateArrivalTime + "]"
    val validSourceline4 = "[1,\"" + randomStopRoute2.id.value + "\",\"" + f.testBusRoute2.id.value + "\",2,\"Any Place\",\"V23456\"," + f.generateArrivalTime + "]"
  ...
  test("Vehicle actors should add incoming lines relating to arrival times to stop record"){f=>
    ...
    eventually {
      ...
      f.historicalSourceLineProcessor.getCurrentActors.futureValue.size shouldBe 1
      f.historicalSourceLineProcessor.getArrivalRecords(VehicleReg("V123456"), f.testBusRoute1).futureValue.size shouldBe testLines.size
    }
    dataStreamProcessorTest.stop
  }
  ...
  test("Vehicle actors should update record when closer arrival time is received"){f=>
    ...
    val testLines = routeDefFromDb.map(busStop =>
      "[1,\"" + busStop.id.value + "\",\"" + f.testBusRoute1.id.value + "\",1,\"Any Place\",\"V123456\"," + arrivalTime + "]")
    ...
    eventually {
      ...
      f.historicalSourceLineProcessor.getCurrentActors.futureValue.size shouldBe 1
      f.historicalSourceLineProcessor.getArrivalRecords(VehicleReg("V123456"), f.testBusRoute1).futureValue.apply(routeDefFromDb.head) shouldBe newArrivalTime
    }
    dataStreamProcessorTest.stop
  }
  ...
  test("Vehicle actors should persist record if stop arrival time information is complete and passes validation"){f=>
    val routeDefFromDb = f.definitions(f.testBusRoute1)
    val vehicleReg = VehicleReg("V123456")

    val testLines: List[String] = routeDefFromDb.map(busStop =>
      "[1,\"" + busStop.id.value + "\",\"" + f.testBusRoute1.id.value + "\",1,\"Any Place\",\"" + vehicleReg.value + "\"," + f.generateArrivalTime + "]")
  ...
  test("Vehicle actors should persist record if there is a gap at the beginning of the sequence (bus started midway through route)"){f=>
    ...
    eventually {
      ...
      f.testHistoricalRecordsCollection.getHistoricalRecordFromDbByBusRoute(f.testBusRoute1).filter(result => result.vehicleID == vehicleReg).head.stopRecords.map(record => SeqNo(routeDefFromDb.indexWhere(x => x.id == record.stopID) + 1)) shouldEqual f.testHistoricalRecordsCollection.getHistoricalRecordFromDbByBusRoute(f.testBusRoute1).filter(result => result.vehicleID == vehicleReg).head.stopRecords.map(record => record.seqNo)
    }
    dataStreamProcessorTest.stop
  }
  ...
  test("Only one record should be persisted if multiple persists are requested for the same bus making the same route at the same time"){f=>
    ...
    val testLines1: List[String] = routeDefFromDb.map(busStop =>
      "[1,\"" + busStop.id.value + "\",\"" + f.testBusRoute1.id.value + "\",1,\"Any Place 1\",\"" + vehicleReg.value + "\"," + f.generateArrivalTime + "]")
  ...
  def validatedSourceLineBackToLine(sourceLine: ValidatedSourceLine): String = {
    ...
    "[1,\"" + sourceLine.busStop.id.value + "\",\"" + sourceLine.busRoute.id.value + "\"," + directionToInt(sourceLine.busRoute.direction.value) + ",\"" + sourceLine.destinationText + "\",\"" + sourceLine.vehicleReg.value + "\"," + sourceLine.arrival_TimeStamp + "]"
  }
}
